...
public class LeaderElectionOverwriteNewerLogTest : FSMTestBase {
    ...
    public void TestLeaderElectionOverwriteNewerLog() {
        ...
        Assert.Multiple(() => {
            Assert.That(CompareLogEntries(fsm1.Log, fsm3.Log, 1, 2), Is.True);
            Assert.That(CompareLogEntries(fsm1.Log, fsm4.Log, 1, 2), Is.True);
            Assert.That(CompareLogEntries(fsm1.Log, fsm5.Log, 1, 2), Is.True);
        });
    }
}
